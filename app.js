...
mongoose.connect("mongodb+srv://prathamesh27:Patu2772@cluster0.ckoedio.mongodb.net/todolistDB");
...
app.get("/",function(req,res){
    ...
    Item.find({},function(err, foundResult){
        if(foundResult.length===0){
            ...
            res.render("list",{listTitle:day,newlistItem:foundResult});
        }
    });
});
...
app.post("/",function(req,res){
    ...
    // Item.insert(itemName,function(err){
    //     if(err){
    //         console.log(err);
    //     }else{
    ...
    if(listName===day){
        Item.deleteOne({_id:deleteItem},function(err){
            if(err){
                ...
                console.log("Item delete from list");
            }
        });
        res.redirect("/");
    }else{
